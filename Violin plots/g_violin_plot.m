...
%
% Usage:
%   g_violin_plot('all')               % All patients
%   g_violin_plot('SE_damaging')       % Only with skin damaging SE strains
%   g_violin_plot('SE_nondamaging')    % Only without skin damaging SE strains
%
% Author: Jamie Lee
% Date: 7 October 2025

function g_violin_plot(filter_mode)
    ...
    asymp_raw = readmatrix('../Analyse steady states/data/asymp.csv');
    rev_raw = readmatrix('../Analyse steady states/data/reversible.csv');
    irrev_raw = readmatrix('../Analyse steady states/data/irreversible.csv');
    ...
    switch filter_mode
        ...
        case 'SE_nondamaging'
            ...
            reversible = rev_raw(rev_raw(:, 19) == 0, :);
            irreversible = irrev_raw(irrev_raw(:, 19) == 0, :);
            filter_display = 'SE Non-Damaging';
    ...
    % Add title with proper formatting (no subscripts from underscores)
    title_text = sprintf('Parameter Distributions Across Patient Types (%s)', filter_display);
    ...
    output_file = fullfile(figures_folder, sprintf('ViolinPlots_%s.png', filter_mode));
